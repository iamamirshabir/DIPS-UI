...
export class PhysicianDialogComponent implements OnInit {
  ...
  days: number[];
  availability: string='';
  invalidDays: number[]=[6,6,6,6,6,6,6];
  validDays: number[]=[];
  ...
  myDateFilter = (d: Date | null): boolean => {
    ...
    return day !== this.invalidDays[0] && day !== this.invalidDays[1] && day !== this.invalidDays[2] && day !== this.invalidDays[3] && day !== this.invalidDays[4] && day !== this.invalidDays[5] && day !== 6;
  }
  onNoClick(): void {
  ...
  makeAppointment(){
    ...
    this.physicianDialogService.setAppointment(this.appointment,
      this.userService.userAc.userac_id,this.data.id).subscribe((resp: any)=>
      {  
        if(resp!= null){
